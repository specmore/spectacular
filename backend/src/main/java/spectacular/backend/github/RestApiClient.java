...
public class RestApiClient {
  ...
  public SearchCodeResults findFiles(String filename, String fileExtension, String path,
                                     String org) {
    ...
    ResponseEntity<SearchCodeResults> response = restTemplate
        .exchange(uriComponentsBuilder.build().toUriString(), HttpMethod.GET, entity,
            SearchCodeResults.class);
    return response.getBody();
  }

  public boolean isUserRepositoryCollaborator(Repository repo, String username) {
    UriComponentsBuilder uriComponentsBuilder =
        UriComponentsBuilder.fromUriString(REPO_COLLABORATORS_PATH);

    HttpHeaders headers = new HttpHeaders();
    HttpEntity entity = new HttpEntity(headers);

    String contentUri =
        uriComponentsBuilder.buildAndExpand(repo.getNameWithOwner(), username).toUriString();
    ResponseEntity<Void> response =
        restTemplate.exchange(contentUri, HttpMethod.GET, entity, Void.class);
  ...
  public spectacular.backend.github.domain.Repository getRepository(Repository repo) {
    ...
    ResponseEntity<spectacular.backend.github.domain.Repository> response = restTemplate
        .exchange(contentUri, HttpMethod.GET, entity,
            spectacular.backend.github.domain.Repository.class);
    return response.getBody();
  }
  ...
  public GraphQLResponse graphQLQuery(GraphQLRequest graphQLRequest) {
    ...
    ResponseEntity<GraphQLResponse> response =
        restTemplate.postForEntity(GRAPH_QL, entity, GraphQLResponse.class);
    return response.getBody();
  }
